...
${persona.lens}
RULES:
- Use ONLY the supplied engine metrics and public context. Never invent financial figures.
- Write a real memo, not a slogan. argument = 5–8 sentences. valuationTake = 3–4 sentences. Each thesis bullet = 2 sentences.
- Return ONLY a JSON object with keys: id, thesis (3-5 strings), valuationTake, argument, catalysts (2-4), risks (2-4).
- id must be "${persona.id}".`,
  ...
  return personaNarrativeSchema.parse({ ...parsed, id: personaId });
}
...
export async function generateDebate(
  ...
  const digest = narratives
    .map((n) => `${n.id.toUpperCase()}: ${n.argument}\nValuation: ${n.valuationTake}`)
    .join("\n\n");
  const text = await complete(
    `You are the IC secretary. Write a heated debate (6-8 turns) where Buffett, Thiel, the PE partner, and Dalio argue AGAINST each other using the engine figures. Each turn is 3–5 sentences. Then a 4–6 sentence chairSummary.
...
export async function generateIcAnalysis(
  ...
  const results = await Promise.allSettled(
    PERSONAS.map((p) => generatePersonaNarrative(p.id, bundle, ctx)),
  );
  ...
  const { debate, chairSummary } = await generateDebate(ordered, bundle, ctx);

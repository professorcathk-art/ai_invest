...
Top-nav also has **Top-Down 行業研報 / Sector Research** (`/industry-research`) and **私募與併購 / Private Market** (`/private-market`). Sector research is a **dated digest**: sector and **general breaking** headlines stay on the tape even when they omit a ticker (tariffs, oil, export controls). Names are only attached when the title actually says them. The 06:15 HKT desk note then infers which watchlist names are helped or hurt — that is not an IC vote, and generic wires are never stamped with NVDA or COST. Pick a calendar date to reopen a stored day. Private-market rows are AI-digested into **one company per deal**; the target is plain text and source links live in a Sources column. The page stays blank when the public feed is empty — no invented Sequoia / a16z / KKR rows. The stock workbench **一圖讀懂公司業務 / Business Breakdown** tab uses FMP product / geographic segmentation when present; product cards still fill from the Yahoo description.
